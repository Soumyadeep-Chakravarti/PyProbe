...
def _discover_str_data_offset() -> int:
    """
    Dynamically finds the memory offset where a Compact ASCII string's 
    character array begins by scanning for exact byte values.
    
    Handles PEP 623 (removal of wstr in 3.12+) automatically.
    """
    # Create a fresh stringDD
    s = "".join(["A", "B", "C", "D"])
    s_addr = id(s)
    
    # Target raw byte representations
    expected_first_byte = b'A'
    expected_second_byte = b'B'
    
    # State flags end at +36, so we scan from there up to +80
    for offset in range(36, 80):
        try:
            # Read a single C-char (1 byte) from memory
            val = ctypes.c_char.from_address(s_addr + offset).value
            
            if val == expected_first_byte:
                # Double-check the next byte to prevent false positives
                next_val = ctypes.c_char.from_address(s_addr + offset + 1).value
                if next_val == expected_second_byte:
                    return offset
        except Exception:
            continue
...
print(f"String OK: {_fmt_offset(STR_DATA_OFFSET)}")
...
if __name__ == "__main__":
    ...
    print(f"  String items offset     : {_fmt_offset(STR_DATA_OFFSET)}")
    print()
    print("  Dict layout:")
